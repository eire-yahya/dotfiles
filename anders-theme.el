;;; anders-themes.el --- based on ander murphy's personal theme
...
(deftheme anders
  "Following typographic standards, to its deepest, and supreme abilities")

(defun anders/hsl->hex (H S L)
...
(defvar anders/hue 180)
(setq anders/hue 30)
(defun anders/gen-dark-theme ()
  "Generate a dark theme."
  (let ((hue anders/hue))
    `((bg          . ,(anders/hsl->hex hue  11  18))
      (bg-inactive . ,(anders/hsl->hex hue  13  20))
      (bg-region   . ,(anders/hsl->hex hue  12  28))
      (fg          . ,(anders/hsl->hex hue  14  71))
      (fg-comment  . ,(anders/hsl->hex hue   9  55))
      (fg-dim      . ,(anders/hsl->hex hue  10  40))
      (blue        . ,(anders/hsl->hex hue 100  66))
      (cyan        . ,(anders/hsl->hex 207  82  65))
      (green       . ,(anders/hsl->hex  95  38  62))
      (red         . ,(anders/hsl->hex 355  65  65))
      (yellow      . ,(anders/hsl->hex  39  67  69))
      (purple      . ,(anders/hsl->hex 286  60  67)))))
(defvar anders/dark-theme (anders/gen-dark-theme))

(defun anders/gen-light-theme ()
  "Generate a light theme."
  (let ((hue anders/hue))
    `((bg          . ,(anders/hsl->hex hue  11  (- 110 18)))
      (bg-inactive . ,(anders/hsl->hex hue  13  (- 110 20)))
      (bg-region   . ,(anders/hsl->hex hue  12  (- 110 28)))
      (bg-hl       . ,(anders/hsl->hex hue  9  (- 110 20)))
      (fg          . ,(anders/hsl->hex hue  14  (- 90 71)))
      (fg-comment  . ,(anders/hsl->hex hue   9  (- 100 55)))
      (fg-dim      . ,(anders/hsl->hex hue  10  (- 100 40)))
      (blue        . ,(anders/hsl->hex hue 100  (- 100 66)))
      (cyan        . ,(anders/hsl->hex 221  87  60))
      (green       . ,(anders/hsl->hex 119  34  47))
      (red         . ,(anders/hsl->hex   5  74  59))
      (yellow      . ,(anders/hsl->hex  41  99  38))
      (purple      . ,(anders/hsl->hex 301  63  40)))))
(defvar anders/light-theme (anders/gen-light-theme))

(defun anders/set-theme-faces (theme-colors)
  ...
  (let* ((fg          (assoc-default  'fg          theme-colors))
         ...
         (bg-hl       (assoc-default  'bg-hl       theme-colors))
    ...
    (custom-theme-set-faces
     ...
     `(font-lock-function-name-face ((t (:background ,bg :foreground ,fg :weight bold))))
     ...
     `(tuareg-font-lock-governing-face ((t (:foreground ,fg :background ,bg))))
     `(tuareg-font-lock-operator-face ((t (:foreground ,fg :background ,bg))))
     )))

(defvar anders/active-theme)
(setq anders/active-theme anders/light-theme)

(defun anders/toggle-dark-light-theme ()
  "Toggle theme between dark and light."
  (interactive)
  (setq anders/active-theme
        (if (eq anders/active-theme anders/dark-theme)
            anders/light-theme
          anders/dark-theme))
  (anders/set-theme-faces anders/active-theme)
  (enable-theme 'anders))

(defun anders/gen-theme ()
  "Generate a new theme."
  (interactive)
  (setq anders/hue 30)
    (setq anders/dark-theme (anders/gen-dark-theme))
    (setq anders/light-theme (anders/gen-light-theme))
    (call-interactively 'anders/toggle-dark-light-theme))

(anders/set-theme-faces anders/active-theme)
(provide-theme 'anders)
;;; anders-theme.el ends here
